...
#[tokio::main]
async fn main() {
    let endpoint = std::env::var("EDGESTORE_S3_ENDPOINT_URL").expect("set EDGESTORE_S3_ENDPOINT_URL to a running LocalStack");
    let bucket = std::env::var("EDGESTORE_S3_BUCKET").unwrap_or_else(|_| "pierre-bench".to_string());
    ...
    for &n in &[100usize, 1_000, 10_000] {
        ...
        for i in 0..n {
            ...
            pierre::ingest::commit(&storage, wire, &[], None, None).await.unwrap();
        }
        ...
        println!("  n={n:>6} records/segment  upload: {:>8.2?}  ({:>8.0} records/sec)", upload_elapsed, n as f64 / upload_elapsed.as_secs_f64());
    }
    ...
    for &n in &[100usize, 1_000, 10_000] {
        ...
        let backup_config = BackupConfig::S3 { bucket: bucket.clone(), prefix: Some(prefix.clone()), endpoint: Some(endpoint.clone()) };
        let remote = pierre::backup::build_remote_store(backup_config).await.unwrap();
        let storage = Storage::open_with_remote(source_dir.path(), remote).await.unwrap();

        for i in 0..n {
            let wire = WireRecord { timestamp_ns: i as i64, message: format!("cold record {i}"), fields: Default::default() };
            pierre::ingest::commit(&storage, wire, &[], None, None).await.unwrap();
        }
        let meta = storage.flush_to_segments().await.unwrap();
        ...
        let backup_config = BackupConfig::S3 { bucket: bucket.clone(), prefix: Some(prefix), endpoint: Some(endpoint.clone()) };
        let remote = pierre::backup::build_remote_store(backup_config).await.unwrap();
        let fresh = Storage::open_with_remote(fresh_dir.path(), remote).await.unwrap();

        let query_start = Instant::now();
        let results = fresh.query_archived_range(&[meta], 0, n as i64 + 1).await.unwrap();
        let query_elapsed = query_start.elapsed();
        assert_eq!(results.len(), n, "cold query must return every record, not a partial/corrupted result");
        println!("  n={n:>6} records  cold-query: {:>8.2?}  ({:>8.0} records/sec)", query_elapsed, n as f64 / query_elapsed.as_secs_f64());
    }

    println!("\n=== Consistency: repeated + concurrent upload/download round-trips ===");
    let dir = tempfile::tempdir().unwrap();
    let backup_config =
        BackupConfig::S3 { bucket: bucket.clone(), prefix: Some("bench-consistency/".to_string()), endpoint: Some(endpoint.clone()) };
    let remote = pierre::backup::build_remote_store(backup_config).await.unwrap();
    let storage = Storage::open_with_remote(dir.path(), remote).await.unwrap();
    ...
    for round in 0..ROUNDS {
        let wire =
            WireRecord { timestamp_ns: round as i64, message: format!("consistency round {round}"), fields: Default::default() };
        pierre::ingest::commit(&storage, wire, &[], None, None).await.unwrap();
        let meta = storage.flush_to_segments().await.unwrap();
        storage.archive_segments(vec![meta.clone()]).await.unwrap();
        segments.push(meta);
    ...
    for meta in segments {
        let path = dir.path().join(format!("segment-{:08}.dat", meta.segment_id));
        let bucket = bucket.clone();
        let endpoint = endpoint.clone();
        handles.push(tokio::spawn(async move {
            let backup_config =
                BackupConfig::S3 { bucket, prefix: Some("bench-consistency/".to_string()), endpoint: Some(endpoint) };
            let remote = pierre::backup::build_remote_store(backup_config).await.unwrap();
            ...
            let scratch = Storage::open_with_remote(scratch_dir.path(), remote).await.unwrap();
            let downloaded = scratch.query_archived_range(&[meta], 0, i64::MAX).await.unwrap();
    ...
    assert_eq!(failures, 0, "every concurrent round-trip must be byte-exact and complete");
