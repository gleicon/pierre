...
async fn warm_segment_is_backed_up_to_s3() {
    ...
    let remote = pierre::backup::build_remote_store(backup_config)
        ...
        .unwrap();
    let storage = Arc::new(
        Storage::open_with_remote(data_dir.path(), remote)
            .await
            .unwrap(),
    );
    ...
    assert!(
        !manifest.is_empty(),
        "flush_to_segments should have produced at least one warm segment"
    );
    ...
    tokio::task::spawn_blocking(move || {
        let verifier = S3RemoteStore::new(&bucket, Some(prefix), Some(&endpoint))
            .expect("build verifier store");
        for (hash, local_bytes) in expected {
            let hex: String = hash.iter().map(|b| format!("{b:02x}")).collect();
            let downloaded = verifier
                .download(&hash)
                .unwrap_or_else(|e| panic!("segment {hex} not found in S3: {e}"));
            assert_eq!(
                downloaded, local_bytes,
                "backed-up bytes must match the local segment exactly"
            );
        }
    })
    .await
