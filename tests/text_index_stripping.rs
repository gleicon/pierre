...
async fn stripping_is_not_yet_durable_across_a_restart_known_upstream_gap() {
...

    {
        ...
        let storage = Storage::open_with_options(dir.path(), Box::new(remote), 3600, true).await.unwrap();

        let mut fields = BTreeMap::new();
        fields.insert("level".to_string(), "error".to_string());
        let wire = WireRecord { timestamp_ns: 1_000_000_000, message: "payment gateway timeout".to_string(), fields };
        pierre::ingest::commit(&storage, wire, &["level".to_string()], None, None).await.unwrap();
        storage.index_text(b"text_bucket_0", b"doc-1", "payment gateway timeout").await.unwrap();
        ...
        assert!(metas[0].text_index_stripped, "segment should be marked stripped");
        assert_eq!(metas[0].record_count, 1, "the __text__ record should be gone from the segment (log record only remains)");
    } // storage dropped — simulates a process exit.

    // Reopen against the same local directory and the same remote — a real restart.
    let remote = edgestore_repl::FilesystemRemoteStore::new(remote_dir.path().to_path_buf()).unwrap();
    let reopened = Storage::open_with_options(dir.path(), Box::new(remote), 3600, true).await.unwrap();
    ...
    let hits_after = reopened.search_text(b"text_bucket_0", "payment", 10).await.unwrap();
    assert_eq!(hits_after.len(), 1, "KNOWN GAP: WAL replay resurrects stripped data on restart — see this test's doc comment");
    ...
    assert_eq!(logs_after.len(), 1, "raw log records must survive text-index stripping untouched");
