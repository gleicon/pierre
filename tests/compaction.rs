...
async fn compaction_removes_expired_cohort_without_touching_live_data() {
    ...
    let storage = Storage::open_with_options(dir.path(), Box::new(remote), 1, false)
        .await
        .unwrap();

    let ns = b"compaction_test";
    storage
        .put_with_ttl(ns, b"expiring", b"short-lived", 1)
        .await
        .unwrap();
    storage
        .put_with_ttl(ns, b"persisting", b"long-lived", 100)
        .await
        .unwrap();
    ...
    assert_eq!(
        storage.get(ns, b"expiring").await.unwrap(),
    ...
    assert_eq!(
        storage.get(ns, b"expiring").await.unwrap(),
        None,
        "expired cohort should be gone after compaction"
    );
    assert_eq!(
        storage.get(ns, b"persisting").await.unwrap(),
        Some(b"long-lived".to_vec()),
