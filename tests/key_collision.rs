...
async fn same_timestamp_records_get_distinct_keys_and_both_survive() {
    ...
    for i in 0..1000 {
        ...
        let record = Record { timestamp_ns: same_ts, message: format!("record {i}"), fields, template_id: 0 };
        let key = storage.commit(&record).await.unwrap();
        keys.push(key);
    }

    let unique_keys: std::collections::HashSet<_> = keys.iter().collect();
    assert_eq!(unique_keys.len(), 1000, "1000 records at the identical timestamp must all get distinct keys");
    ...
    assert_eq!(results.len(), 1000, "all 1000 same-timestamp records must survive, none silently overwritten");
}
