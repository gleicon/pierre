...
async fn backup_worker_prunes_local_segment_after_grace_period_and_data_stays_queryable() {
    ...
    let storage = std::sync::Arc::new(Storage::open_with_remote(dir.path(), Box::new(remote)).await.unwrap());

    let mut fields = BTreeMap::new();
    fields.insert("level".to_string(), "error".to_string());
    let wire = WireRecord { timestamp_ns: 1_000_000_000, message: "prune me after archiving".to_string(), fields };
    ...
    assert_eq!(count_dat_files(dir.path()), 1, "segment should be flushed and archived, but not yet prunable (grace period not elapsed)");

    // Wait comfortably past the grace period for the prune tick to catch it.
    tokio::time::sleep(Duration::from_millis(1500)).await;
    assert_eq!(count_dat_files(dir.path()), 0, "local segment file should be deleted once its grace period has elapsed");

    // Data must still be queryable — read-through to the archived copy.
    let results = storage.range(0, 2_000_000_000).await.unwrap();
    assert_eq!(results.len(), 1, "pruned segment's data must still be reachable via archived read-through");
